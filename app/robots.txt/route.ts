...
export async function GET(request: NextRequest): Promise<NextResponse> {
  // Block all non-production hosts (staging, PR previews) from indexing.
  // GSC reported ~7,800 staging URLs leaking into Google's index because
  // robots.txt allowed everything. Detect via request Host header so the
  // same code can serve production and non-production from a single build.
  // Default-deny: anything not on the production allowlist is noindexed.
  const requestHost = request.headers.get("host") || "";
  const isNonProductionHost = !isProductionHost(requestHost);

  if (isNonProductionHost) {
    const body = [
      `# Generated by route handler at ${new Date().toISOString()}`,
      `# Non-production host (${requestHost}) — all crawlers disallowed`,
      "",
      "User-Agent: *",
      "Disallow: /",
      "",
    ...
    return new NextResponse(body, {
      ...
      headers: {
        ...
        "Cache-Control": getCacheControlHeader(request, 3600, 86400),
        "X-Robots-Source": "route-handler-v2-noindex",
      },
    });
  }
