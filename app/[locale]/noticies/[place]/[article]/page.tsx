...
export async function generateMetadata({
  ...
  params: Promise<{ locale: AppLocale; place: string; article: string }>;
}): Promise<Metadata> {
  const { locale, place, article } = await params;
  ...
  if (detail) {
    const base = buildPageMeta({
      ...
      canonical: toLocalizedUrl(`/noticies/${place}/${article}`, locale),
