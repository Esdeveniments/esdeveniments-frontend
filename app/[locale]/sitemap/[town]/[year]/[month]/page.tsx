...
export async function generateMetadata({
  ...
  params: Promise<MonthStaticPathParams & { locale: AppLocale }>;
}) {
  ...
  return buildPageMeta({
    ...
    canonical: toLocalizedUrl(`/sitemap/${town}/${year}/${monthSlug}`, locale),
    locale,
  });
}
...
export default async function Page({
  ...
  const { locale, town, year, month } = await params;
  const t = await getTranslations({ locale, namespace: "Pages.SitemapMonth" });
  const tConstants = await getTranslations({
    locale,
    namespace: "Components.Constants",
