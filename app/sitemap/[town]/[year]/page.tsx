...
import type { EventsByTownYearResponseDTO } from "types/api/event";

export default function TownYearSitemap({
  events,
}: {
  events: EventsByTownYearResponseDTO[];
}): JSX.Element {
  return (
    <>
      <Meta
        ...
        canonical={`${siteUrl}/sitemap/${events[0]?.town.toLowerCase()}/${events[0]?.year}`}
      />
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const events = await fetchEventsByTownYear(
