# Agent setup for `que-fer`

This is a deepsec scanning workspace. Project `que-fer` is registered
(target: `..`). Use this prompt when `data/que-fer/INFO.md` needs a
refresh after major architecture, auth, API, or deployment changes.

## What to do

1. **Read the deepsec skill.** After `yarn install`, the file is at
...
2. **Fill in `data/que-fer/INFO.md`.** It's auto-injected into the AI
...
```bash
yarn deepsec scan    --project-id que-fer
yarn deepsec process --project-id que-fer
```
