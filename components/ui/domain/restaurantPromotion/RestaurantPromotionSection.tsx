...
export default function RestaurantPromotionSection({
  ...
  return (
    <div
      ref={sectionRef}
      className="flex w-full items-start justify-center gap-component-sm px-container-x lg:px-container-x-lg"
    >
      {/* Loading State */}
      {isLoading && (
        <Skeleton
      ...
      {/* Error State */}
      {error && (
        <div className="rounded-lg border border-error/50 bg-error/10 p-component-md">
          <Text as="p" variant="body-sm" className="text-error">
            {error}
          </Text>
        </div>
      )}
