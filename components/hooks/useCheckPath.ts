...
export const useCheckPath = (): boolean => {
  ...
  const truePaths: readonly string[] = [
    "/",
    "/[place]",
    "/[place]/[byDate]",
  ] as const;
  ...
  const isTruePath: boolean = truePaths.some((path: string): boolean =>
    pathname.startsWith(path),
  );

  // Check if the current path matches any of the false paths
  const isFalsePath: boolean = falsePaths.some((path: string): boolean =>
    pathname.startsWith(path),
  );
