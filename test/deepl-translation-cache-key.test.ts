...
import { testing } from "../app/e/[eventId]/actions";
...
describe("DeepL translation cache key", () => {
  it("is deterministic for the same (text, lang)", () => {
    ...
    const k1 = testing.getCacheKey(text, "en");
    const k2 = testing.getCacheKey(text, "en");
    expect(k1).toBe(k2);
  });

  it("differs when target language differs", () => {
    const text = "a càrrec de Carme Porcel - Lloc: Sala d'Actes de la Biblioteca";
    const enKey = testing.getCacheKey(text, "en");
    const esKey = testing.getCacheKey(text, "es");
  ...
  it("differs for different text (sanity check)", () => {
    const t1 = "Hola món";
    const t2 = "Adeu món";
    const k1 = testing.getCacheKey(t1, "en");
    const k2 = testing.getCacheKey(t2, "en");
    expect(k1).not.toBe(k2);
  });

  it("produces a compact hash segment (16 hex chars)", () => {
    const text = "Lorem ipsum dolor sit amet";
    const hash = testing.hashText(text);
