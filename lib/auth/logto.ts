...
export function mapUserInfoToAuthUser(info: LogtoUserInfo): AuthUser {
  const profileSlug = info.custom_data?.profileSlug;
  return {
    id: info.sub,
    email: info.email ?? "",
    displayName: info.name ?? info.username ?? undefined,
    avatarUrl: info.picture ?? undefined,
    profileSlug: typeof profileSlug === "string" ? profileSlug : undefined,
